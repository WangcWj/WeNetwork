...
`WeNetWork`采用的是单例模式，也就是`OkHttp`和`Retrofit`对象基本上只会创建一次（动态添加拦截器的话会重新创建一次）。单例模式的目的也是方便维护，和解决网络请求发起时的对象创建过程，但是全局单例会持续占用一点内存，也是一种时间换空间的做法。另外单例模式可以让网络的发起变得简单化，很适合链式请求。单例模式带来的问题还是很多的：
